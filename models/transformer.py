...
from models.utils import PositionEmbeddingSine


class Transformer(nn.Module):
    def __init__(self, inp_res=32, dim=256, num_heads=4, mlp_ratio=4., injection=True):
        super().__init__()
        ...
        self.layer = Block(dim=dim, num_heads=num_heads, mlp_ratio=mlp_ratio, injection=injection)
    ...
    def forward(self, query, key):
        ...
        output = self.layer(query=output, key=key)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=1., act_layer=nn.GELU, norm_layer=nn.LayerNorm, injection=True):
        super().__init__()
    ...
    def forward(self, query, key, query_embed=None, key_embed=None):
        ...
        # print(key)
        pe_key = self.pe(key)
        query_embed = repeat(pe_query, '() n c d -> b n c d', b=b)
        key_embed = repeat(pe_key, '() n c d -> b n c d', b=b)
...
if __name__ == '__main__':
    ...
    a = torch.rand((1, 256, 32, 32)).to('cuda')
    model(a, a)
